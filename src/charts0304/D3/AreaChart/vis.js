...
const draw = (props) => {
   ...
   // let colorset = d3.scaleOrdinal(d3.schemeCategory10);
    ...
    if (hasSeries) {
        y.domain([0, d3.max(stackedData[stackedData.length - 1], d => d[1])]).nice().range([height-30, 0]);
    } else{
        y.domain([0, d3.max(data, function (d) { return d[encoding.y.field]; })]).nice().range([height-30, 0]);
    }
