...
const getCategories = (rawData, encoding) => {
    ...
    for (let i = 0; i < rawData.length; i++) {
        if (dataCategories[rawData[i][encoding.Hierarchy.field]]) {
            dataCategories[rawData[i][encoding.Hierarchy.field]].push(rawData[i]);
        }
        else {
            dataCategories[rawData[i][encoding.Hierarchy.field]] = [rawData[i]];
        }
    }
...
const getStackedData = (rawData, encoding) => {
    ...
    for (const s in dataSeries) {
        ...
        for (let index = 0; index < dataSeries[s].length; index++) {
            ...
            // console.log(rowData);
            dataSeriesCategories[s][rowData[encoding.Hierarchy.field]] = rowData[encoding.size.field]
        }
    ...
    console.log('prearedData',preparedData)
...
const getMinRows = (rawData, encoding) => {
    let calculateData = d3.nest().key(d => d[encoding.Hierarchy.field]).entries(rawData);
