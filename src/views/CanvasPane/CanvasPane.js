...
export default class CanvasPane extends PureComponent {
  ...
  static defaultProps = {
    ...
    generateDOM = (e) => {
      ...
      let suggestion = []
      suggestion.push('rgb(88,218,232)')
        ...
        return (
          ...
          <div ref='carddiv' className="carddiv" style={{backgroundColor: this.props.cardColor, border:!this.props.chartMode&&this.props.editMode==i? '1px solid '+`${this.props.textColor}`:'1px solid '+`${this.props.shadowColor}`, textAlign: 'center',pointerEvents:'box-none'}} key={item.i} data-grid={item} onClick={()=>this.selectChart(i, item.spec)}>
            <input type="text" id="myVal"  
             ...
             placeholder={this.props.widgets[i].type}
            value={this.props.widgets[i].inputvalue} onChange={this.inputChange.bind(this,i)} 
            ...
            {component}
            <div style={{position:'absolute',bottom:'0', width:'100%',display:'flex',justifyContent:'center',marginLeft:item.type=='heat map'?'-40px':''}}>
                    ...
                    <div style={{width:'40px'}}>
                      ...
                      <Col span={2}>
                        {/* <Tooltip title={fieldsItem}> */}
                          <div style={{width:item.type=='heat map'||item.type=='map'? '150px':'20px',textAlign:'left',marginTop:'-14px',color:this.props.textColor,whiteSpace:item.type=='bar chart'||'piechart'?'nowrap':'',textOverflow:item.type=='bar chart'||'piechart'?'ellipsis':'',overflow:item.type=='bar chart'||'piechart'?'hidden':'', fontSize:'8px'}} title={fieldsItem}>
                            {fieldsItem}
